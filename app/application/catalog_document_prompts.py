...
2. content 문서의 상품 자체 정보는 상품명에서 확인되는 사실만 쓴다.
   질감, 향의 강도, 성능, 품질, 후기, 효능을 지어내지 않는다.
3. usage 와 gift 문서에서는 상품 종류에 맞는 사용 상황과 추천 대상을 다양하게 추론해도 된다.
   단, 추론한 상황이나 대상을 상품 자체의 스펙이나 성능처럼 쓰지 않는다.
...
10. 상품명에 직접 없는 숫자, 용량, 개수, 소재, 구성품, 성능을 만들지 않는다.
11. "프리미엄", "고급형", "품질이 좋다"처럼 상품명으로 확인할 수 없는 평가를 쓰지 않는다.
12. usage 에는 상품 종류와 자연스럽게 연결되는 활동, 장소, 상황을 다양하게 쓸 수 있다.
13. gift 에는 상품 종류와 자연스럽게 연결되는 직업, 취미, 생활 방식, 관계를 다양하게 쓸 수 있다.
14. 예를 들어 핸드크림의 사용 장소나 추천 대상을 추론하는 것은 허용하지만,
    상품명에 없는 용량, 개수, 질감, 보습 성능을 상품 사실로 추가하는 것은 금지한다.

## attributes 작성 기준

- 알 수 없는 값은 추측하지 말고 null 로 쓴다.
- needsSize 는 의류·신발처럼 구매자가 규격을 골라야 하는 상품만 true 로 쓴다.
  용량은 사이즈 선택이 아니다.
- needsInstall 은 상품명에 설치·조립이 명시되거나 설치가 필수인 상품만 true 로 쓴다.
- materials 에는 상품명에 직접 등장한 소재만 넣는다. 소재가 없으면 빈 배열로 쓴다.
  핸드크림, 키보드, 머그컵 같은 상품 종류는 소재가 아니다.
- hasScent 와 containsAlcohol 은 상품명에서 직접 확인될 때만 true 로 쓴다.
...
_EXAMPLE_ASSISTANT = json.dumps(
    {
        ...
        "contentDocument": "티타늄 소재의 350ml 더블월 구조 머그컵.",
        "usageDocument": (
